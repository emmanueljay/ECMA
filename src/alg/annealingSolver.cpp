...
double get_energy(const Solution& sol, int version = 0) {
  ...
  switch (version) {
    case 0: {
      ...
      ratio = sol.ratio();
      if (ratio < 2)
        energy += sol.data_.n * sol.data_.n * 1000 * (1 + 2 - ratio);
      break;
    }
    case 1: {
...
bool AnnealingSolver::solve(double init_temp, bool use_sol) {
  ...
  while (diff < FLAGS_maxTime && temperature > 0.0001) {
    ...
    if (k % FLAGS_sizePalier == 0)
      temperature =
          get_next_temperature(temperature, FLAGS_lambda, FLAGS_variant);
    VLOG_IF(4, k < 20) << "Temperature value " << temperature;

    // 5. We choose if we keep the change or not.
    if (energy_neighbour < energy ||
        ((double)rand() / (RAND_MAX)) <
            exp((energy - energy_neighbour) / temperature)) {
      VLOG_IF(4, k < 20) << "AnnealingSolver:: New solution found, energy = "
                         << energy_neighbour;
      if (energy_neighbour < energy)
        status += "-";  // Better
      else
      ...
      if (energy < energy_max) { // We have a better solution
        VLOG(3) << status;
        status = "";
        VLOG(2) << "AnnealingSolver:: Better sol found : " << energy
                << " Temp : " << temperature;
        best_sol.fill_sol(sol_);
        energy_max = energy;
      }
